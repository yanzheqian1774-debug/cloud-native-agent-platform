"""Bounded Kimi Responses adapter for governed Draft Assistance.

The Kimi projection is deliberately separate from the OpenAI projection even
though both use a Responses-shaped HTTPS endpoint.  This module never reads
environment variables, follows redirects, retries, or persists raw content.
"""
...
ADAPTER_ID = "kimi-responses-draft"
ADAPTER_REVISION = "v1"
...
class KimiResponsesConfiguration:
    execution_class: str
    responses_url: str
    native_model_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.execution_class not in {"REAL_PROVIDER", "LOCAL_HTTPS_MOCK"}
            ...
            raise DraftAssistanceError("KIMI_RESPONSES_PROFILE_INVALID")
        text_values = (
...
class ResolvedKimiCredential:
    ...
    def __repr__(self) -> str:
        return "ResolvedKimiCredential(<redacted>)"


class ExactFileKimiCredentialResolver:
    def __init__(
        self,
...
class KimiResponsesDraftTransport:
    """One-shot Kimi transport with a Kimi-owned request projection."""

    def __init__(self, configuration: KimiResponsesConfiguration) -> None:
        self.configuration = configuration
    ...
    def _observation_id(invocation_id: str, correlation: str, kind: str) -> str:
        ...
        return f"kimi-response-observation:{digest}"
    ...
    def _interpret_response(
        ...
        if status == "in_progress":
            return ProviderObservation(
                self._observation_id(invocation_id, correlation, status),

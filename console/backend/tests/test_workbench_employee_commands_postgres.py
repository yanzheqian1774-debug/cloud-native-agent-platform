...
DATABASE_URL = os.environ.get("S5_310_EMPLOYEE_WORKBENCH_TEST_DATABASE_URL")
pytestmark = pytest.mark.skipif(
    not DATABASE_URL, reason="310 Employee Workbench PostgreSQL 15 required"
)
...
def generation(now: datetime) -> StaticAuthorityGeneration:
    ...
    return StaticAuthorityGeneration(
        ...
        audit_source="impl-310-employee-workbench-test",
        credentials=(
...
def employee_workbench():
    ...
    database_name = f"impl310_employee_{uuid.uuid4().hex}"
    ...
    *,
    ...
    return administrator.post(
        ...
        json={
            ...
            "decision": decision,
            "reasonCategory": (
                "ASSIGNED_BUSINESS_DUTY"
                if decision == "APPROVE"
                else "REQUEST_NOT_JUSTIFIED"
            ),
            "basisType": "TICKET",
            "basisReference": "SEC-310-FORMAL",
            **(
...
def test_public_employee_lifecycle_uses_session_csrf_and_exact_current_grants(
    ...
    assert denied_member.status_code == 404, denied_member.json()
...
def test_rejection_executes_nothing_and_stale_decision_keeps_original_version(
    employee_workbench,
) -> None:
    applicant = TestClient(
        employee_workbench.application, base_url="https://console.example"
    )
    administrator = TestClient(
        employee_workbench.application, base_url="https://console.example"
    )
    applicant_csrf = login(applicant, "formal-secret")
    administrator_csrf = login(administrator, "admin-secret")
    requested = request_grants(
        applicant,
        applicant_csrf,
        (ExactGrant("EMPLOYEE", "CREATE", "employee:collection"),),
        key="rejected-create-request",
    )
    assert requested.status_code == 202
    request_id = requested.json()["requestId"]

    rejected = decide_request(
        administrator,
        administrator_csrf,
        request_id,
        key="rejected-create-decision",
        decision="REJECT",
    )
    assert rejected.status_code == 201
    assert rejected.json()["state"] == "REJECTED"
    assert rejected.json()["aggregateVersion"] == 2

    stale = decide_request(
        administrator,
        administrator_csrf,
        request_id,
        key="stale-create-decision",
        decision="APPROVE",
        expected_version=1,
    )
    assert stale.status_code == 409
    assert stale.json()["reasonCode"] == "AUTHORIZATION_STATE_STALE"
    inspected = applicant.get(f"{PREFIX}/authorization/grant-requests/{request_id}")
    assert inspected.status_code == 200
    assert inspected.json()["state"] == "REJECTED"
    assert inspected.json()["aggregateVersion"] == 2

    employee_id = "employee-definition:rejected-create"
    denied = applicant.post(
        f"{PREFIX}/employees",
        json=create_body(
            employee_workbench,
            command_id="employee-command:rejected-create",
            definition_id=employee_id,
        ),
        headers=unsafe_headers(applicant_csrf),
    )
    assert denied.status_code == 404
    assert denied.json()["reasonCode"] == "AUTHORIZATION_NOT_FOUND"
    with psycopg.connect(employee_workbench.database_url) as connection:
        assert (
            connection.execute(
                "SELECT count(*) FROM digital_employee_definition.definitions "
                "WHERE definition_id=%s",
                (employee_id,),
            ).fetchone()[0]
            == 0

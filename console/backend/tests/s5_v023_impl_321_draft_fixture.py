"""Synthetic, exact-model Draft Assistance fixture for S5-V023-IMPL-321."""
...
from agent_console.draft_assistance_policy import V2_SCHEMA_VERSION, policy_for
...
from agent_console.openai_responses_draft_adapter import ADAPTER_ID
...
def build_fixture(
    ...
    *,
    ...
    provider = ModelProviderRevision(
        scope,
        "provider:s5-321-synthetic",
        "provider-revision:s5-321:1",
    ...
    endpoint = ModelEndpointRevision(
        scope,
        "endpoint:s5-321-synthetic",
        "endpoint-revision:s5-321:1",
    ...
    connection = ModelConnectionProfileRevision(
        scope,
        "connection-profile:s5-321-synthetic",
        "connection-profile-revision:s5-321:1",
        endpoint.identity,
        SecretReference(
            "secret-reference:s5-321-mock"
            if real_provider
            else "secret-reference:s5-321-none",
    ...
    for ordinal, action in enumerate(
        ...
        repository.append_fact(
            ModelLifecycleFact(
                scope,
                f"model-fact:s5-321:{ordinal}",
                revision.identity,
                ordinal,
                action,
                "human:fixture-reviewer",
                f"decision:s5-321:{ordinal}",
                now,
            )
        )
    repository.close()
    ...
    pepper_path.write_bytes(b"s5-321-synthetic-pepper-value-01")
    ...
    runtime_document = {
        ...
        "profileRevisionId": "draft-profile-revision:s5-321:1",
        "policyDigest": policy_for("v2", V2_SCHEMA_VERSION).digest,
        "profileDigest": _sha("draft-profile:s5-321:1"),
        "model": {
        ...
        runtime_document.update(
            {
                ...
                "nativeModelId": "mock-model-321",
                "maximumInputTokens": 10_000,
                "maximumResponseBytes": 64_000,
                "connectTimeoutSeconds": 2,
                "readTimeoutSeconds": 3,
                "credential": {
                    "reference": "secret-reference:s5-321-mock",
                    "version": "mock-v1",
                ...
                "budget": {
                    "ledgerId": "s5-v023-impl-321-mock-provider",
                    "currency": "USD",
                    "callCap": 20,

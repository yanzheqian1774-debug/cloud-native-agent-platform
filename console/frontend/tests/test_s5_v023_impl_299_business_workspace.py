...
def test_native_https_harness_is_explicit_and_default_paths_remain_plain() -> None:
    ...
    assert '"**/digital-employee-work-participation.real.spec.ts"' in config
...
def test_grant_administration_is_a_separate_exact_request_page() -> None:
    ...
    admin = text("digital-employees/EmployeeGrantAdministrationPage.tsx")
    app = text("App.tsx")
    assert (
        'path="/authorization-admin" element={<EmployeeGrantAdministrationPage/>}'
        in app
    )
    assert "decideGrantRequest" not in applicant
    assert "decideEmployeeGrantRequest" in admin
    assert "requestId.trim()" in admin
    assert 'request.purpose !== "WORKBENCH_EMPLOYEE_LIFECYCLE"' in admin
    assert "不把 URL、query 或 purpose 当作授权依据" in admin
    assert "不是数字员工生命周期的业务 APPROVE" in admin

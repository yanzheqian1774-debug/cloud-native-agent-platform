...
const applicantCredential = required("REL_317_APPLICANT_CREDENTIAL");
const administratorCredential = required("REL_317_ADMIN_CREDENTIAL");
...
test("REL-317 combines trusted Problem, Criteria, and Employee configuration through visible controls", async ({ browser }, testInfo) => {
  ...
  await test.step("Employee CREATE is frozen, independently authorized, and does not imply READ", async () => {
    ...
    await applicant.getByLabel("职责角色").fill("REL-317 可信准备负责人");
    ...
    await expect(applicant.getByLabel("创建命令确认")).toContainText("REL-317 可信准备负责人");
  ...
  await test.step("Exact Employee and Agent READ plus lifecycle grants close the lifecycle", async () => {
    await authorizeEmployeeRequest(applicant, administrator);
    await expect(applicant.getByRole("heading", { name: "REL-317 可信准备负责人", exact: true })).toBeVisible();

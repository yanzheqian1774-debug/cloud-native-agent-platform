...
test("unknown commands preserve payload and new input while access denial and stale CAS fail closed",async({page})=>{
  ...
  await page.reload();await page.getByLabel("已保存成功标准").getByRole("button",{name:"修订此标准"}).click();await page.getByLabel("修改成功标准原文").fill("人工确认连续交付恢复。");await page.getByRole("button",{name:"采用标准原文"}).click();state.unknownSetOnce=true;await page.getByRole("button",{name:"确认并保存"}).click();await expect(page.getByLabel("成功标准待确认卡片")).toContainText("结果未知");const setCommand=state.setWrites.at(-1);const supplement=page.getByLabel("待处理补充（仅本页）");await supplement.fill("保存期间的新补充不应被清空。");state.holdSet=true;const recovery=page.getByRole("button",{name:"恢复原保存结果"}).click();await expect.poll(()=>state.setWrites.length).toBe(3);await supplement.focus();state.continueSet?.();await recovery;await expect(supplement).toHaveValue("保存期间的新补充不应被清空。");await expect(supplement).toBeFocused();expect(state.setWrites.at(-1)).toEqual(setCommand);

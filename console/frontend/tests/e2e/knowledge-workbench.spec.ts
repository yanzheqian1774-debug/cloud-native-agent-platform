...
test("completes the real Knowledge lifecycle, retrieval, recovery and purge journey", async ({ page, request }, testInfo) => {
  ...
  await expect(page.getByLabel("Knowledge information hierarchy")).toContainText("日常检索与引用");
  await expect(page.getByLabel("Knowledge information hierarchy")).toContainText("质量评估");
  await expect(page.getByLabel("Knowledge information hierarchy")).toContainText("导入与重复项审查");
  await expect(page.getByLabel("Knowledge information hierarchy")).toContainText("索引重建、清除与恢复");

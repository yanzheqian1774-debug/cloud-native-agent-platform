...
async function authorize(page:Page,admin:Page){const card=page.getByLabel('AI 问题理解与草稿辅助');await expect(card).toContainText('AUTHORIZATION_PENDING');await decide(admin,await fact(page,'辅助授权申请'));await card.getByRole('button',{name:'使用已保留正文继续 AI 调用',exact:true}).click();await expect.poll(()=>fact(page,'模型授权申请')).not.toBe('尚未提交');await decide(admin,await fact(page,'模型授权申请'));await card.getByRole('button',{name:'使用已保留正文继续 AI 调用',exact:true}).click();await expect(page.getByLabel('问题草稿卡片').getByRole('button',{name:'确认创建',exact:true})).toBeEnabled();}
...
test('real authority denial prevents local provider dispatch',async({browser,request})=>{
 ...
 await page.locator('#problem-composer').fill('321授权拒绝，不应dispatch');await page.locator('#problem-composer').press('Enter');const card=page.getByLabel('AI 问题理解与草稿辅助');await expect(card).toContainText('AUTHORIZATION_PENDING');await decide(admin,await fact(page,'辅助授权申请'),false);await card.getByRole('button',{name:'使用已保留正文继续 AI 调用',exact:true}).click();await expect(page.getByRole('alert')).toContainText('DRAFT_ASSISTANCE_NOT_FOUND');

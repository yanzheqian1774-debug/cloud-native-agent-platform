...
export function TraceabilityProjection({context,perspective,data,error,retry}:{context:CanonicalUrlContext;perspective:"product"|"technical";data:TraceabilityDTO|null;error:ProductAssemblyError|null;retry:()=>void}) {
  ...
  return <section className="traceability-projection">
    ...
    {perspective==="product"?<div className="traceability-list">{data.claims.map(claim=><article tabIndex={-1} id={`claim-${claim.claimKey}`} key={claim.claimKey}><h2>{claim.productLabel}</h2><span className={`badge ${claim.status==="SUPPORTED"?"":"warning"}`}>{claim.status}</span>{claim.limitationCodes.map(code=><code key={code}>{code}</code>)}<div>{claim.evidenceRefs.map(id=><Link key={id} to={resourceViewLink(withUrlContext(context,{evidenceId:id,claimKey:claim.claimKey}),"evidence")}>Evidence {id}</Link>)}</div>{claim.technicalFactKeys.map(key=><Link key={key} to={resourceViewLink(withUrlContext(context,{factKey:key,claimKey:claim.claimKey}),"technical")}>Technical fact {key}</Link>)}</article>)}</div>:<div className="traceability-list">{data.technicalFacts.map(fact=><article tabIndex={-1} id={`fact-${fact.factKey}`} key={fact.factKey}><h2>{fact.factKey}</h2><span className="badge">{fact.valueClassification}</span><pre>{JSON.stringify(fact.provenance,null,2)}</pre>{fact.affectedClaimKeys.map(key=><Link key={key} to={resourceViewLink(withUrlContext(context,{claimKey:key,factKey:fact.factKey}),"product")}>Affected claim {key}</Link>)}</article>)}</div>}
  </section>;
}

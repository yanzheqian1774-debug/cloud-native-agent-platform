...
const supplierQuality = { title: "Supplier Quality Analyst", duties: ["analyze supplier quality"], data: ["quality records"], knowledge: ["supplier quality policy"], skills: ["root cause analysis"], capabilities: ["supplier-quality-analysis"], runtimes: ["native"] };
...
export function AgentWorkbenchPage() {
  ...
  return <main className="agent-workbench"><header><p className="eyebrow">Enterprise Resource Workbench</p><h1>Agent Workbench</h1><p>Create, review, publish and inspect canonical Agent Definitions. PostgreSQL is authoritative; browser state is never lifecycle authority.</p></header>
    ...
    <section className="agent-detail">{!definition?<div className="agent-empty"><h2>Select a Definition</h2><p>Its lifecycle, exact digests, relationships and history appear here.</p></div>:<><header><p className="eyebrow">{definition.lifecycleState} · {definition.enabled?"Enabled":"Disabled"}</p><h2>{definition.name}</h2><span className="technical-value">{definition.definitionId}</span></header>
      ...
      <section><h3>Lifecycle actions</h3><div className="agent-actions">{definition.publishedRevisionId&&!definition.currentDraftRevisionId&&<button onClick={()=>act(value=>successorAgentDefinition(value.definitionId,value.aggregateVersion))}>Create successor draft</button>}<button onClick={()=>act(value=>lifecycleAgentDefinition(value.definitionId,value.enabled?"DISABLE":"ENABLE",value.aggregateVersion))}>{definition.enabled?"Disable":"Enable"}</button>{definition.publishedRevisionId&&<button onClick={()=>act(value=>lifecycleAgentDefinition(value.definitionId,"DEPRECATE",value.aggregateVersion))}>Deprecate</button>}<button onClick={()=>act(value=>lifecycleAgentDefinition(value.definitionId,"ARCHIVE",value.aggregateVersion))}>Archive</button></div></section>
      ...
      <AgentTechnicalProjection projection={selected}/></>}</section></div>
  </main>;
}

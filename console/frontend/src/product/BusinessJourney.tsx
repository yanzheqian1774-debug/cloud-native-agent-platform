...
export function BusinessJourney({ fixture, state, dispatch }: Props) {
  ...
  return <section id="work" className="product-section journey-panel" aria-labelledby="journey-title">
    ...
    {state.step !== "QUESTION" && <div className="plan-preview"><div className="section-heading"><h2>{t("product.plan.title")}</h2><p className="stable-id">{state.revision}</p></div><ol>{fixture.taskKeys.map((key, i) => <li key={key}><span>{i + 1}</span><div><strong>{t(key as never)}</strong><p>{t("product.plan.previewTask")}</p></div></li>)}</ol><p className="honesty-note">{t("product.instances.honesty")}</p></div>}
    {(state.step === "EXECUTION" || state.step === "OUTCOME") && <div className="progress-box"><div className="progress-heading"><strong>{t("product.execution.progress")}</strong><span className="stable-id">{state.execution}</span></div><progress value={state.execution === "COMPLETED" ? 100 : state.execution === "RUNNING" ? 62 : 0} max="100" /><p>{t("product.execution.synthetic")}</p></div>}

...
export function UserMessage({children,occurredAt,authorLabel="你"}:{children:ReactNode;occurredAt?:string;authorLabel?:string}){return <article className="px-message px-user-message"><div className="px-avatar" aria-hidden="true">H</div><div><span className="px-message-author">{authorLabel}</span>{occurredAt&&<time dateTime={occurredAt}>{formatTime(occurredAt)}</time>}<p>{children}</p></div></article>}

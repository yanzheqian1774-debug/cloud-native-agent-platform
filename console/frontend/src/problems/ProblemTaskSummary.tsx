...
function SummaryContent({revision,exact,criteria,sets,authorization,busy,readFailed,copied,onCopy,onLocateProblem,onLocateAuthorization}:{revision?:BusinessProblemRevision;exact?:BusinessProblemRevision;criteria:CriterionRevision[];sets:CriteriaSetRevision[];authorization:AuthorizationSummary;busy:boolean;readFailed:boolean;copied:boolean;onCopy:()=>void;onLocateProblem:()=>void;onLocateAuthorization:()=>void}){
  ...
  return <div className="px-task-summary-content">
    ...
    <section><span className="px-eyebrow">已保存成功标准</span>{highest?<><h3>{members.length} 项正式成员</h3><p>{members.map(item=>String(item.measurement.rubric??item.criterion_type)).join("；")}</p><dl><dt>显示依据</dt><dd>正式返回历史中的最高修订，不代表自动选择</dd><dt>成功标准集合修订</dt><dd>{highest.revision}</dd><dt>集合标识</dt><dd><code>{highest.set_revision_id}</code></dd></dl><button type="button" onClick={()=>document.getElementById("saved-success-criteria-message")?.scrollIntoView({behavior:"smooth",block:"center"})}>定位成功标准</button></>:<p>尚无正式成功标准集合响应；本地草稿和页内补充不会出现在这里。</p>}</section>
  </div>;
}

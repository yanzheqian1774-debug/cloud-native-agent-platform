...
type Props={revision?:BusinessProblemRevision;detail?:BusinessProblemDetail;criteria:CriterionRevision[];sets:CriteriaSetRevision[];authorization:AuthorizationSummary;busy:boolean;readFailed:boolean;contextKey:string;onLocateProblem:()=>void;onLocateAuthorization:()=>void};

const latest=<T extends {revision:number}>(values:T[])=>[...values].sort((a,b)=>b.revision-a.revision)[0];

export function ProblemTaskSummary({revision,detail,criteria,sets,authorization,busy,readFailed,contextKey,onLocateProblem,onLocateAuthorization}:Props){
  ...
  const content=<SummaryContent revision={formal} exact={exact} criteria={criteria} sets={sets} authorization={authorization} busy={busy} readFailed={readFailed} copied={copiedContext===contextKey} onCopy={()=>void copyRequest()} onLocateProblem={()=>locate(onLocateProblem)} onLocateAuthorization={()=>locate(onLocateAuthorization)}/>;
...
function SummaryContent({revision,exact,criteria,sets,authorization,busy,readFailed,copied,onCopy,onLocateProblem,onLocateAuthorization}:{revision?:BusinessProblemRevision;exact?:BusinessProblemRevision;criteria:CriterionRevision[];sets:CriteriaSetRevision[];authorization:AuthorizationSummary;busy:boolean;readFailed:boolean;copied:boolean;onCopy:()=>void;onLocateProblem:()=>void;onLocateAuthorization:()=>void}){
  ...
  return <div className="px-task-summary-content">
    <section><span className="px-eyebrow">问题摘要</span>{revision?<><h3>{revision.title}</h3><p>{exact?exact.description:readFailed?"本次读取暂时失败，旧的问题详情已撤下。":"获得查看权限后显示问题详情。"}</p><dl><dt>修订</dt><dd>{revision.revision}</dd><dt>授权状态</dt><dd>{authorizationState}</dd><dt>内容读取</dt><dd>{readState}</dd></dl><button type="button" onClick={onLocateProblem}>定位问题消息</button><details><summary>技术详情</summary><p>问题详情仅在 fresh exact GET 成功后显示；摘要不会用本地草稿补全正式响应。</p></details></>:<p>确认创建后，这里显示正式响应中的问题摘要。</p>}</section>
    <section><span className="px-eyebrow">查看申请</span><h3>{authorizationState}</h3><p>{next}</p>{request&&<><code>{request.requestId}</code><button type="button" onClick={onCopy}>复制申请编号</button><span className="px-copy-status" aria-live="polite">{copied?"已复制":""}</span></>} {(request||continuation)&&<button type="button" onClick={onLocateAuthorization}>定位授权消息</button>}</section>
    <section><span className="px-eyebrow">已保存成功标准</span>{highest?<><h3>{members.length} 项正式成员</h3><p>{members.map(item=>String(item.measurement.rubric??item.criterion_type)).join("；")}</p><dl><dt>显示依据</dt><dd>正式返回历史中的最高修订，不代表自动选择</dd><dt>Criteria Set 修订</dt><dd>{highest.revision}</dd><dt>Set ID</dt><dd><code>{highest.set_revision_id}</code></dd></dl><button type="button" onClick={()=>document.getElementById("saved-success-criteria-message")?.scrollIntoView({behavior:"smooth",block:"center"})}>定位成功标准</button></>:<p>尚无正式 Criteria Set 响应；本地草稿和页内补充不会出现在这里。</p>}</section>
  </div>;
}

...
export function GrantAdministrationPage(){
  ...
  return <main className="px-page px-admin-page"><header className="px-page-title"><div><p>授权管理 <small>Exact Grant Administration</small></p><h1>授权申请审批</h1><span>只处理你输入的精确申请编号；本页不提供申请列表或搜索。</span></div><a href="/work">返回申请人工作台</a></header>
    ...
    {request&&<section className="px-admin-card"><span className="px-eyebrow">当前状态</span><h2>{request.state==="PENDING"?"等待处理":request.state==="APPROVED"?"已批准":"已拒绝"}</h2><p>{request.state==="PENDING"?"请核对申请用途和请求操作，再作出一次明确决定。":request.state==="APPROVED"?"该申请已经批准。申请人可返回原工作台刷新授权状态。":"该申请已经拒绝。申请人页面不会读取问题正文。"}</p><dl><dt>申请编号</dt><dd><code>{request.requestId}</code></dd><dt>状态版本</dt><dd>{request.aggregateVersion}</dd><dt>申请用途</dt><dd>{request.purpose}</dd><dt>请求操作</dt><dd>{request.requestedActions.join(", ")}</dd></dl>{request.state==="PENDING"?<div className="px-admin-actions"><button disabled={busy} onClick={()=>decide("REJECT")}>拒绝申请</button><button className="px-primary-button" disabled={busy} onClick={()=>decide("APPROVE")}>批准精确读取</button></div>:<p className="px-boundary-note">这是不可覆盖的终态。再次检查同一申请编号可恢复当前决定。</p>}</section>}
  </main>;
}

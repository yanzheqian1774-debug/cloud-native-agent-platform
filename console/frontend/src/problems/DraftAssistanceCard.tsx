...
import type { DraftAssistanceResult, DraftReadiness } from "../api/businessWorkspace";
...
export function DraftAssistanceCard({result,compact=false,contentAvailable=false,canResubmit=true,originalText,onSuccessor,busy,onAnswer,onRefresh,onObserve,onCancel,onReject,onManualFallback}:Props){
  ...
  const permissionsCurrent=readiness?.invocationId===result.invocationId&&readiness.permissionsCurrent;
  ...
  const statusLabel=pending&&!contentAvailable?"正文待恢复":pending&&permissionsCurrent?"等待继续":clarification?"等待补充信息":({AUTHORIZATION_PENDING:"等待授权",REQUESTED_AWAITING_CONTENT:"等待正文重交",OUTCOME_UNKNOWN:"结果待确认",CANCELLATION_REQUESTED:"取消已请求，停止未确认",CANCELLATION_CONFIRMED:"收到取消确认",REJECTED_BY_USER:"已拒绝",SUCCEEDED:draftReady?"草稿已生成":"辅助已完成",FAILED:"辅助失败",FAILED_PRE_DISPATCH:"派发前未通过"} as Record<string,string>)[result.state]??"状态已更新";
  ...
  return <Wrapper {...(compact&&result.settlementStatus&&!["NOT_MEASURED","ESTIMATE_SETTLED"].includes(result.settlementStatus)?{open:true}:{})} id="draft-assistance-message" tabIndex={-1} className="px-inline-card px-assistance-card" aria-label="AI 问题理解与草稿辅助">
    ...
    <header><div><span className="px-eyebrow">{clarification?"AI 需要你补充":"AI 建议草稿（尚未创建）"}</span><h2>{pending?(!contentAvailable?"原正文在本页不可用":permissionsCurrent?"授权已核验，等待继续":"等待精确授权"):unknown?"调用结果尚不确定":result.resultKind==="NEEDS_CLARIFICATION"?"需要补充信息":result.resultKind==="DRAFT_READY"?"可修改草稿已生成":"辅助状态已更新"}</h2></div><span className={`px-status ${clarification?"warning":finished?"success":result.state.includes("REJECT")||result.state.includes("FAIL")?"danger":"warning"}`}>{statusLabel}</span></header>
    ...
    {pending&&<div className="px-mode-note"><strong>{permissionsCurrent?"授权已核验；继续前仍会重新校验": "待核验当前准入，不是模型超时"}</strong><p>草稿辅助与模型调用分别保留正式授权记录。登录成功不会自动取得这些权限。审批人请在独立浏览器配置或隐私窗口打开链接；同浏览器的新标签不隔离账号，不要在本业务会话切换身份。</p><p><a target="_blank" rel="noreferrer" href={`/authorization-admin?context=${encodeURIComponent(result.contextId)}`}>核对本 context 的独立调用准入</a></p>{result.requestAuthorizationRequestId&&<p><a target="_blank" rel="noreferrer" href={`/authorization-admin?request=${encodeURIComponent(result.requestAuthorizationRequestId)}`}>查看本次草稿辅助申请</a></p>}{result.modelAuthorizationRequestId&&<p><a target="_blank" rel="noreferrer" href={`/authorization-admin?request=${encodeURIComponent(result.modelAuthorizationRequestId)}`}>查看本次模型调用申请</a></p>}</div>}
    {pending&&<><DraftAuthorizationStatus key={result.invocationId} result={result} onReadiness={setReadiness}/>{contentAvailable?<><p>原始提交正文仍保留在本页内存中，输入框清空不代表正文丢失。查询授权不会重交；下方单独按钮才会请求实际调用。离开或刷新后正文不能从服务端恢复。</p><details><summary>查看本页保留的用户原文</summary><p>{originalText}</p></details></>:<p role="status">原始提交正文在本页不可用，服务端只保存元数据和校验承诺，无法恢复原文。本次不能重交；保留原对象与摘要，可明确填写关联后继，不能冒充原正文。</p>}</>}

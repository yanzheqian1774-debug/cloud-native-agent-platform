...
export function DraftAssistanceCard({result,busy,onRefresh,onObserve,onCancel,onReject,onManualFallback}:Props){
  ...
  const statusLabel=({AUTHORIZATION_PENDING:"等待授权",REQUESTED_AWAITING_CONTENT:"等待正文重交",OUTCOME_UNKNOWN:"结果待确认",CANCELLATION_REQUESTED:"正在请求取消",CANCELLATION_CONFIRMED:"已取消",REJECTED_BY_USER:"已拒绝",SUCCEEDED:"草稿已生成",FAILED:"辅助失败"} as Record<string,string>)[result.state]??"状态已更新";
  return <section id="draft-assistance-message" tabIndex={-1} className="px-inline-card px-assistance-card" aria-label="AI 问题理解与草稿辅助">
    ...
    <p className="px-truth-note">{result.transport==="SYNTHETIC"?"本次使用明确标识的模拟辅助生成，没有调用真实 AI 服务。":"本次草稿来自已授权的真实 AI 服务；仍需人工核对和确认。"}</p>

...
function state(problem:ProblemPlan){if(problem.status.includes("FAIL"))return ["执行失败","danger"] as const;if(problem.approval)return ["计划已批准","success"] as const;if(problem.planRevisions.length)return ["等待人工处理","warning"] as const;return ["未执行","neutral"] as const}
...
function ProblemDetail({problem}:{problem:ProblemPlan}){
  const revision=problem.planRevisions.at(-1),interpretation=problem.interpretations.at(-1),[label,tone]=state(problem);
  return <><header className="px-work-header"><div><span className={`px-status ${tone}`}>{label}</span><h2>{problem.name}</h2><small>{problem.displayCode} · 修订 {problem.revision}</small></div><Link to="/problems">打开审核与调整</Link></header>
    <article className="px-message user"><span>业务问题</span><p>{problem.description}</p></article>
    <section className="px-work-card"><Heading eyebrow="成功标准" title="业务目标与验证条件"/>{interpretation?<dl className="px-criteria"><div><dt>对象范围</dt><dd>{interpretation.supplierScope||"未配置"}</dd></div><div><dt>时间范围</dt><dd>{interpretation.timeRange||"未配置"}</dd></div><div><dt>改善指标</dt><dd>{interpretation.improvementIndicator||"未配置"}</dd></div></dl>:<Empty title="未配置" copy="尚无已确认的成功标准。"/>}</section>
    <article className="px-message employee"><span>数字员工工作响应</span><p>{revision?.summary||"尚未形成可审核计划。"}</p><small>{revision?`Plan Revision ${revision.displayCode}`:"未配置"}</small></article>
    <section className="px-work-card"><Heading eyebrow="Workflow / Plan" title={revision?`${revision.tasks.length} 个计划任务`:"计划尚未形成"} status={revision?"已配置":"未配置"}/>{revision?<ol className="px-task-flow">{revision.tasks.map((task,index)=><li key={task.taskId}><b>{index+1}</b><div><strong>{task.name}</strong><small>{task.displayCode} · 负责角色 {task.role}</small><ResourceSummary task={task}/></div><span className="px-status neutral">未执行</span></li>)}</ol>:<Empty title="未配置" copy="当前没有 Workflow / Plan。"/>}</section>
    <section className="px-work-card"><Heading eyebrow="Human Intervention" title="人工介入与审批" status={problem.approval?"已批准":"等待人工处理"}/><p>{problem.approval?"审批已绑定精确计划修订与摘要，但不代表已经执行。":"需要人工核验计划、能力缺口和成功标准后再决定是否批准。"}</p><Link className="px-secondary-button" to="/problems">{problem.approval?"查看审批记录":"前往审核计划"}</Link></section>
    <div className="px-result-grid"><section className="px-work-card"><span className="px-eyebrow">证据 Evidence</span>{problem.knowledge.citations.length?<><strong>{problem.knowledge.citations.length} 项知识依据</strong><p>这是规划采用的知识引用，不是执行成功证据。</p></>:<Empty title="未执行" copy="尚无执行 Evidence。"/>}</section><section className="px-work-card"><span className="px-eyebrow">结果 Outcome</span><Empty title="未执行" copy="当前投影没有业务 Outcome，不推断成功。"/></section></div>
  </>
}
...
function Heading({eyebrow,title,status}:{eyebrow:string;title:string;status?:string}){return <div className="px-section-heading"><div><span className="px-eyebrow">{eyebrow}</span><h3>{title}</h3></div>{status&&<span className={`px-status ${status==="等待人工处理"?"warning":status==="已批准"?"success":status==="已配置"?"info":"neutral"}`}>{status}</span>}</div>}

...
function DemoPrimaryNavigation() {
  ...
  const items = [["/workspace", "工作台"], ["/tasks", "任务"], ["/employees", "数字员工"], ["/resources", "能力与资源"], ["/runtime", "运行环境"]];
  return <><div className="demo-environment"><span>受控供应商质量演示</span><small>NON_PRODUCTION_DEMO · 确定性规则</small></div><nav className="demo-primary-nav" aria-label="演示主导航">{items.map(([to,label])=><NavLink key={to} to={to} aria-current={pathname===to?"page":undefined}>{label}</NavLink>)}</nav></>;
}
...
function RelationshipGraph({ journey, task }: { journey: LivePlanningJourney; task: JourneyTaskProjection }) {
  const identity=journey.successor.identity; const outcome=journey.successor.outcome;
  const nodes=[
    {type:"TASK",id:task.taskId,label:task.title,icon:"▣"},
    ...(task.matchedRole?[{type:"DIGITAL_EMPLOYEE",id:`employee:${task.definitionId??task.requiredRole}`,label:task.requiredRole,icon:"👤"}]:[]),
    ...(task.definitionId?[{type:"AGENT_DEFINITION",id:task.definitionId,label:task.requiredRole,icon:"🤖"}]:[]),
    ...task.actions.slice(0,1).map(id=>({type:"BUSINESS_CAPABILITY",id,label:id,icon:"🧭"})),
    ...task.skills.slice(0,1).map(id=>({type:"SKILL",id,label:id,icon:"🧰"})),
    ...task.mcpCapabilities.slice(0,1).map(id=>({type:"MCP_CAPABILITY",id,label:id,icon:"🔌"})),
    ...task.knowledgeRefs.slice(0,1).map(id=>({type:"KNOWLEDGE",id,label:id,icon:"📚"})),
    ...task.runtimeRefs.slice(0,1).map(id=>({type:"RUNTIME",id,label:id,icon:"⚙"})),
    ...(identity.platformExecutionIdentity?[{type:"EXECUTION_IDENTITY",id:identity.platformExecutionIdentity,label:"本次执行身份",icon:"◎"}]:[]),
    ...identity.evidenceIds.slice(0,1).map(id=>({type:"EVIDENCE",id,label:"执行证据",icon:"◇"})),
    ...(outcome?[{type:"OUTCOME",id:outcome.outcomeId,label:"业务结果",icon:"✓"}]:[]),
  ];
  const verbs=["由其负责","采用定义","具备","通过其实现","调用","引用","运行于","承载为","产生","支撑"];
  const [selected,setSelected]=useState(0); const node=nodes[selected]??nodes[0];
  return <section className="relationship-panel" aria-label="对象语义关系"><div className="semantic-graph">{nodes.map((item,index)=><span className="semantic-node-wrap" key={`${item.type}:${item.id}`}>{index>0&&<span className="semantic-edge">{verbs[index-1]??"关联"} →</span>}<button className={selected===index?"semantic-node selected":Math.abs(selected-index)===1?"semantic-node related":"semantic-node"} aria-pressed={selected===index} onClick={()=>setSelected(index)}><span aria-hidden="true">{item.icon}</span><strong>{item.label}</strong><small>{item.type}</small></button></span>)}</div>{node&&<aside className="relationship-detail"><p className="eyebrow">当前选择</p><h3>{node.label}</h3><p>上游与下游节点使用当前任务快照中的直接关系高亮；未采集的对象不会补造。</p><details><summary>查看精确技术身份</summary><span className="technical-value">{node.type} · {node.id}</span></details><Link to={technicalQuery(journey,task,node.type,node.id)}>打开完整对象详情 →</Link></aside>}</section>;
}
...
function EmployeesPage({ journeyId }: { journeyId: string | null }) {
  ...
  const tabs=[["overview","概览"],["tasks","负责的任务"],["capabilities","业务能力"],["tools","Skill与工具"],["knowledge","Knowledge"],["runtime","Runtime"],["records","执行记录"]];
...
function App() {
  ...
  if (supplierQualityLive) {
    return <BrowserRouter><SelectedExecutionContext><ConsoleShell><DemoPrimaryNavigation/><Routes><Route path="/" element={<Navigate to="/workspace" replace />} /><Route path="/workspace" element={<WorkspacePage journeyId={supplierQualityJourneyId}/>} /><Route path="/tasks" element={<TaskCenterPage journeyId={supplierQualityJourneyId}/>} /><Route path="/employees" element={<EmployeesPage journeyId={supplierQualityJourneyId}/>} /><Route path="/resources" element={<ResourcesPage journeyId={supplierQualityJourneyId}/>} /><Route path="/runtime" element={<RuntimePage journeyId={supplierQualityJourneyId}/>} /><Route path="/product" element={<ProductViewPage supplierQualityJourneyId={supplierQualityJourneyId ?? undefined} onJourneyStarted={setSupplierQualityJourneyId} />} /><Route path={technicalPath} element={<TechPage supplierQualityJourneyId={supplierQualityJourneyId ?? undefined} questionFirst />} /><Route path="*" element={<Navigate to="/workspace" replace />} /></Routes></ConsoleShell></SelectedExecutionContext></BrowserRouter>;
  }

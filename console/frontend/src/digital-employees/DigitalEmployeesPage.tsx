...
async function readInstanceDefinition(value: EmployeeInstance, signal?: AbortSignal) {
  ...
  if (definition.employeeDefinitionId !== reference.employeeDefinitionId || definition.employeeDefinitionRevisionId !== reference.employeeDefinitionRevisionId || definition.employeeDefinitionDigest !== reference.digest) {
    throw new DigitalEmployeeRequestError("INSTANCE_DEFINITION_IDENTITY_MISMATCH", 409);
  }
  return definition;
}

function verifyAssignmentInstance(value: EmployeeAssignment, instanceId: string) {
...
export function DigitalEmployeesPage() {
  ...
  useEffect(() => {
    ...
    getEmployeeInstance(knownInstanceId, controller.signal).then(async value => {
      ...
      const assignmentValue = knownAssignmentId
        ? verifyAssignmentInstance(await getEmployeeAssignment(knownInstanceId, knownAssignmentId, controller.signal), value.instanceId)
        : null;
  ...
  async function readInstanceExact(id = instanceId.trim()) {
    ...
    try { const value = await getEmployeeInstance(id); if (value.instanceId !== id) throw new DigitalEmployeeRequestError("INSTANCE_IDENTITY_MISMATCH", 409); const definition = await readInstanceDefinition(value); if (turn !== workGeneration.current) return; setInstance(value); setInstanceDefinition(definition); setAssignment(null); setWorkError(null); setWorkState("READY"); updateUrl({ instanceId: value.instanceId, assignmentId: "", placementId: "", attemptId: "", agentInstanceId: "" }); }
    catch (reason) { if (turn === workGeneration.current) { setInstance(null); setInstanceDefinition(null); setAssignment(null); failWork(reason); } }
  }
  async function instantiate() {
    if (!selected || !instanceId.trim()) return;
    const turn = ++workGeneration.current; setWorkState("SAVING");
    try { const created = await createEmployeeInstance(selected, instanceId.trim()); const value = await getEmployeeInstance(created.instanceId); if (value.instanceId !== created.instanceId) throw new DigitalEmployeeRequestError("INSTANCE_IDENTITY_MISMATCH", 409); const definition = await readInstanceDefinition(value); if (turn !== workGeneration.current) return; setInstance(value); setInstanceDefinition(definition); setAssignment(null); setWorkError(null); setWorkState("READY"); updateUrl({ instanceId: value.instanceId, assignmentId: "", placementId: "", attemptId: "", agentInstanceId: "" }); }
    catch (reason) { if (turn === workGeneration.current) { setInstance(null); setInstanceDefinition(null); setAssignment(null); failWork(reason); } }
  }
  async function readAssignmentExact(id = assignmentForm.id.trim()) {
    if (!instance || !id) return;
    const turn = ++workGeneration.current; setWorkState("LOADING");
    try { const value = verifyAssignmentInstance(await getEmployeeAssignment(instance.instanceId, id), instance.instanceId); if (value.assignmentId !== id) throw new DigitalEmployeeRequestError("ASSIGNMENT_IDENTITY_MISMATCH", 409); if (turn !== workGeneration.current) return; setAssignment(value); setWorkError(null); setWorkState("READY"); updateUrl({ instanceId: instance.instanceId, assignmentId: value.assignmentId, placementId: "", attemptId: "", agentInstanceId: "" }); }
    catch (reason) { if (turn === workGeneration.current) { setAssignment(null); failWork(reason); } }
  }
  async function assign() {
    if (!instance) return;
    const turn = ++workGeneration.current; setWorkState("SAVING");
    try { await createEmployeeAssignment(instance.instanceId, { assignmentId: assignmentForm.id, assigneeId: assignmentForm.assignee, businessRole: assignmentForm.role, effectiveFrom: new Date(assignmentForm.from).toISOString(), ...(assignmentForm.until ? { effectiveUntil: new Date(assignmentForm.until).toISOString() } : {}) }); const value = verifyAssignmentInstance(await getEmployeeAssignment(instance.instanceId, assignmentForm.id), instance.instanceId); if (value.assignmentId !== assignmentForm.id) throw new DigitalEmployeeRequestError("ASSIGNMENT_IDENTITY_MISMATCH", 409); if (turn !== workGeneration.current) return; setAssignment(value); setWorkError(null); setWorkState("READY"); updateUrl({ instanceId: instance.instanceId, assignmentId: value.assignmentId, placementId: "", attemptId: "", agentInstanceId: "" }); }
    catch (reason) { if (turn === workGeneration.current) { setAssignment(null); failWork(reason); } }
  }
  ...
  return <main className="px-page px-center-page employee-management">
    ...
    <section className="px-truth-banner" role="status"><span className="px-status neutral">MANAGEMENT PORTS PRESENT</span><strong>管理端口与存储存在；可信浏览器授权尚未闭合</strong><p>Definition / Instance / Assignment / Placement 已有 internal 端口；发布不代表 matching；执行未连接正式 BFF（通用 Runtime 与 Evidence 接线待 305）；Runtime Profile 绑定不代表 Runtime 已启动；Assignment 创建不代表员工正在工作。</p><p>Digital Employee Definition 不等同于 Agent Instance、Runtime Instance 或 Application；本页面执行权限固定为 NONE。</p><p>真实性范围：REAL 仅指服务端管理事实，不代表当前浏览器身份可信；通用执行为 NOT_CONNECTED，未获 Runtime 观测时健康为 UNAVAILABLE。“未执行”只可来自权威执行事实；遥测尚未采集时保持未知。</p></section>
    ...
    {panel === "instance" && <section className="px-center-card employee-form"><h2>实例 · Digital Employee Instance</h2><p>使用精确 Employee Definition ID/revision；Instance 是另一个持久身份。当前正式端口不支持列表，因此这里只覆盖已知 ID。</p><Field label="Instance ID" value={instanceId} set={value => { workGeneration.current += 1; setWorkState("READY"); setInstanceId(value); setInstance(null); setInstanceDefinition(null); setAssignment(null); }} /><div className="agent-actions"><button disabled={!selected?.published || busy || workBusy || !instanceId} onClick={() => void instantiate()}>创建并权威读回</button><button disabled={workBusy || !instanceId} onClick={() => void readInstanceExact()}>按精确 ID 读取</button></div>{instance && <InstanceDetail value={instance} boundDefinition={instanceDefinition} />}</section>}

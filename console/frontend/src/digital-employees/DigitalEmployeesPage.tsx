...
export function DigitalEmployeesPage() {
  ...
  return <main className="px-page px-center-page employee-management">
    <header className="px-page-title employee-page-title"><div><p>数字员工 <small>Digital Employee</small></p><h1>数字员工</h1><span>从员工集合选择一个角色，查看所选修订、装配与工作关系。</span></div><button className="px-primary-button" onClick={() => { setMode("create"); setMobileDetailOpen(false); updateUrl({ mode: "create", detail: "" }); }}>＋ 创建数字员工</button></header>
    <section className="employee-summary-strip" aria-label="当前加载范围"><span><strong>{new Set(items.map(item => item.employeeDefinitionId)).size}</strong> 个员工</span><span><strong>{items.length}</strong> 个修订</span><span><strong>{publishedRevisions}</strong> 个已发布修订</span><small>仅展示当前授权范围内的本批结果；不是全局统计</small></section>
    ...
    {mode === "create" ? <><button type="button" className="employee-back-button" onClick={() => { setMode("employees"); updateUrl({ mode: "employees" }); }}>← 返回员工集合</button><EmployeeDefinitionAssembly agents={agents} hasMoreAgents={Boolean(agentNextCursor)} agentError={agentError ? controlledMessage(agentError) : undefined} onLoadMoreAgents={() => void loadMoreAgents()} onReadback={applyCommandReadback} /></> : <div className={`employee-object-center ${mobileDetailOpen ? "show-mobile-detail" : ""}`}>
      <section className="employee-collection" aria-label="员工集合">
        ...
        <div className="employee-collection-rows" ref={listScroll}>{groups.map(group => {
          ...
          return <article key={group.id} className={`employee-collection-row ${isSelected ? "selected" : ""}`}><button className="employee-row-main" aria-current={isSelected ? "true" : undefined} onClick={() => select(active)}><span className="employee-avatar" aria-hidden="true">员</span><span className="employee-row-copy"><strong>{active.role}</strong><small title={group.id}>{shortIdentity(group.id)}</small></span><span className={`employee-state-chip ${active.publicationState === "PUBLISHED" ? "published" : "draft"}`}>{active.publicationState === "PUBLISHED" ? "已发布" : "未发布"}</span></button><label className="employee-revision-picker"><span>显示版本</span><select aria-label={`${active.role} 选择版本`} value={active.employeeDefinitionRevisionId} onChange={event => { const revision = group.revisions.find(item => item.employeeDefinitionRevisionId === event.target.value); if (revision) select(revision); }}>{group.revisions.map(item => <option key={item.employeeDefinitionRevisionId} value={item.employeeDefinitionRevisionId}>{shortIdentity(item.employeeDefinitionRevisionId)} · {item.publicationState === "PUBLISHED" ? "已发布" : "未发布"}</option>)}</select>{group.revisions.length > 1 && <small>当前结果包含 {group.revisions.length} 个修订；请明确选择</small>}</label></article>;

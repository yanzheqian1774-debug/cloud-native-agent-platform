...
export function AuthorizationAdministrationPage() {
  ...
  return <main className="px-page employee-grant-admin px-admin-page">
    <header className="px-page-title"><div><p>授权管理 <small>Exact Grant Administration</small></p><h1>授权申请决定</h1><span>只处理申请人提供的精确申请编号；展示内容来自正式申请响应，不把 URL、query 或 purpose 当作授权依据。</span></div><a href={returnPath}>返回{problemRequest ? "业务工作台" : "数字员工"}</a></header>

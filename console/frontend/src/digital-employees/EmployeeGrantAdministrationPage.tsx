...
export function AuthorizationAdministrationPage() {
  ...
  const purposeLabel = ({S5_324_RESOURCE_REVIEW:"资源审核与发布",WORKBENCH_EMPLOYEE_LIFECYCLE:"数字员工配置",S5_324_NATIVE_READONLY:"隔离 Native 准备与执行",PROBLEM_DRAFT_ASSISTANCE:"AI 草稿辅助",PROBLEM_DRAFT_MODEL_INVOKE:"模型调用",CONTINUE_PROBLEM_READ:"业务问题读取",WORKBENCH_SUCCESS_CRITERIA:"成功标准"} as Record<string,string>)[request?.purpose??""]??"精确权限";
  ...
  useEffect(()=>{
    if(!session||!requestedId)return;
    let active=true;
    inspectEmployeeGrantRequest(requestedId,workbenchPrincipalKey(session))
      .then(value=>{if(active)setRequest(value)})
      .catch(reason=>{if(active)setError(adminMessage(reason))});
    return ()=>{active=false};
  },[session,requestedId]);
  ...
  return <main className="px-page employee-grant-admin px-admin-page">
    ...
    {session && <>{!requestedId&&<ContextAdmissionPanel key={params.get("context")??""} session={session}/>} {!requestedId&&!params.get("context")&&<TaskDelegationPanel/>}<section className="employee-authorization-request"><h2>检查精确申请</h2><label>申请编号<input value={requestId} disabled={busy} onChange={event => setRequestId(event.target.value)} /></label><p>这是业务申请编号，这不是诊断 ID。未知、不可见、自我决定或缺少 DECIDE 权限的申请不会被进一步区分。</p><button className="px-primary-button" disabled={busy || !requestId.trim()} onClick={() => void inspect()}>{busy ? "正在检查……" : "检查授权申请"}</button></section>
    {error && <p role="alert" className="employee-inline-error">{error}</p>}
    {request && <section className="employee-authorization-request" aria-label="授权申请决定"><span className={`employee-capability-state ${request.state === "APPROVED" ? "available" : request.state === "REJECTED" ? "missing" : "warning"}`}><i />{request.state === "PENDING" ? "等待决定" : request.state === "APPROVED" ? "已批准" : "已拒绝"}</span><h2>{domainLabel} · 核对并处理授权申请</h2><p><strong>本次事项：{purposeLabel}</strong></p><dl><dt>申请人</dt><dd>{request.applicant?.principalId??"当前响应未提供"}</dd><dt>当前审批人</dt><dd>{session.principal.principalId}</dd><dt>组织与范围</dt><dd>{request.applicant?`${request.applicant.tenantId} / ${request.applicant.securityDomain}`:"当前响应未提供"}</dd><dt>申请编号</dt><dd><code>{request.requestId}</code></dd><dt>状态版本</dt><dd>{request.aggregateVersion}</dd><dt>申请用途</dt><dd>{request.purpose}</dd><dt>请求操作</dt><dd>{request.requestedActions.join("、")}</dd></dl>{request.requestedGrants&&<details open><summary>精确授权范围</summary><ul>{request.requestedGrants.map((grant,i)=><li key={i}>{grant.owner} / {grant.action}<br/><code>{grant.resource}</code></li>)}</ul></details>}<p>批准后仅在一小时有效期内允许上述精确操作；本次决定不会自动调用模型、发布资源或执行任务。</p><p className="px-truth-note">这里作出的是 Grant 管理员决定，不是数字员工生命周期的业务 APPROVE。</p>{request.state === "PENDING" ? <div className="employee-form-actions"><button disabled={busy} onClick={() => void decide("REJECT")}>拒绝申请</button><button className="px-primary-button" disabled={busy} onClick={() => void decide("APPROVE")}>批准精确权限</button></div> : <p>这是不可覆盖的终态；申请人可返回原页面刷新并恢复冻结命令。</p>}</section>}</>}
  </main>;
}

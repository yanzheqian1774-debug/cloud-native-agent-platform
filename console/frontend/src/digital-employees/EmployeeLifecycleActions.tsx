...
export function EmployeeLifecycleActions({
  ...
  return <section className="employee-lifecycle" aria-labelledby="employee-lifecycle-title">
    <header className="employee-section-heading"><div><h3 id="employee-lifecycle-title">生命周期操作</h3><p>生命周期状态与动作权限分别呈现；权限在每次提交时按当前 session 校验。</p></div><span className={`employee-capability-state ${lifecycle ? "available" : "warning"}`}><i />{lifecycle ? `当前状态 · ${lifecycle}` : "部分实现 · 状态未披露"}</span></header>

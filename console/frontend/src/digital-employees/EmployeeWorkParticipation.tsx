...
export function EmployeeWorkParticipation({ definition, instance, assignment, initialCoordinates, onCoordinates }: {
  ...
  useEffect(() => () => { generation.current += 1; activeRead.current?.abort(); }, []);
  ...
  useEffect(() => {
    ...
    // Restored URL coordinates are intentionally consumed once after their
    // authoritative Instance and Assignment parents have been read.
  ...
  async function readPlacement() {
    ...
    setState("LOADING");
    try {
      const value = await getEmployeePlacement({ instanceId: instance.instanceId, assignmentId: assignment.assignmentId, ...coordinates }, controller.signal);
      ...
      const value = reason instanceof DigitalEmployeeRequestError ? reason : new DigitalEmployeeRequestError("DIGITAL_EMPLOYEE_UNAVAILABLE", 503);
      setPlacement(null);
  ...
  return <section className="employee-work-participation" aria-labelledby="employee-work-title">
    ...
    <div className="employee-work-stages" aria-label="员工工作参与阶段">
      ...
      <Fact label="实际执行 execution" value={placement ? `${placement.execution.state} · ${placement.execution.reasonCode}` : "未知 / 尚未接通"} state={placement && placement.execution.state !== "UNAVAILABLE" ? "known" : "unknown"} />
      <Fact label="实际终态 terminal" value={placement ? `${placement.outcome.state} · ${placement.outcome.reasonCode}` : "未知 / 尚未接通"} state={placement && placement.outcome.state !== "UNAVAILABLE" ? "known" : "unknown"} />
    </div>
    ...
    <fieldset disabled={!instance || !assignment || state === "LOADING"}>
      ...
      <button type="button" onClick={() => void readPlacement()} disabled={!instance || !assignment || !coordinates.placementId || !coordinates.attemptId || !coordinates.agentInstanceId || state === "LOADING"}>{state === "LOADING" ? "正在权威读取……" : "读取精确工作关联"}</button>
    </fieldset>
    {!instance && <p role="status">先在“实例”页按精确 ID 读取 Instance。</p>}
    {instance && !assignment && <p role="status">再在“工作分配”页按精确 ID 读取 Assignment。</p>}
    {error && <div role="alert" className="qto-alert"><strong>{error.kind}</strong><span>{error.kind === "denied" || error.kind === "not found" ? "资源不可用或当前访问未获授权。" : error.code}</span></div>}
    ...
    {placement && <section className="employee-placement-detail" aria-label="Placement 权威详情">
      <dl className="employee-profile-grid">
        ...
        <div><dt>Attempt / Agent Instance</dt><dd><code>{coordinates.attemptId}</code><br/><code>{coordinates.agentInstanceId}</code></dd></div>
        <div><dt>Runtime Instance</dt><dd><code>{placement.runtimeInstanceId}</code></dd></div>
        <div><dt>观测新鲜度</dt><dd>{placement.observation?.freshness ?? "尚未接通"}{placement.observation?.observationId && <><br/><code>{placement.observation.observationId}</code></>}</dd></div>

...
export function WorkflowSkillOperationBindingEditor({task,readOnly=false,directory=[],directoryState="UNAVAILABLE",directoryReason,onChange}:{task:WorkflowTask;readOnly?:boolean;directory?:WorkflowSkillOperationEntry[];directoryState?:DirectoryState;directoryReason?:string|null;onChange?:(task:WorkflowTask)=>void}){
  ...
  return <section className="workflow-skill-binding" aria-label={`步骤 ${task.taskId} 的 Skill operation 绑定`}>
    ...
    {!readOnly&&<fieldset className="binding-picker" disabled={directoryState!=="READY"||directory.length===0}><legend>{editingIndex===null?"添加精确绑定":"替换精确绑定"}</legend>
      <label>Skill resource<select aria-label={`步骤 ${task.taskId} 选择 Skill`} value={skillId} onChange={event=>{setSkillId(event.target.value);setRevisionId("");setOperationName("")}}><option value="">请选择已发布、启用且合格的 Skill</option>{skills.map(entry=><option key={entry.skillId} value={entry.skillId}>{entry.skillName} · {entry.skillId}</option>)}</select></label>
      <label>Skill revision<select aria-label={`步骤 ${task.taskId} 选择 Skill revision`} value={revisionId} onChange={event=>{setRevisionId(event.target.value);setOperationName("")}}><option value="">请选择精确 revision</option>{revisions.map(entry=><option key={entry.skillRevisionId} value={entry.skillRevisionId}>{entry.skillRevisionId} · {entry.skillDigest}</option>)}</select></label>
      <label>Operation<select aria-label={`步骤 ${task.taskId} 选择 operation`} value={operationName} onChange={event=>setOperationName(event.target.value)}><option value="">请选择正式 operation identity</option>{operations.map(entry=><option key={key(entry)} value={entry.operation.name}>{entry.operation.name}</option>)}</select></label>

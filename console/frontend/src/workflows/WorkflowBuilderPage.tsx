import type { ExactReference, WorkflowContent, WorkflowTask } from "../api/workflowDefinitions";
...
export function WorkflowBuilderPage({content,onChange}:{content:WorkflowContent;onChange:(value:WorkflowContent)=>void}){
  const updateTask=(index:number,patch:Partial<WorkflowTask>)=>onChange({...content,tasks:content.tasks.map((task,position)=>position===index?{...task,...patch}:task)});
  ...
  return <section className="workbench-card resource-form" aria-label="Workflow Builder">
    ...
    {content.tasks.map((task,index)=><fieldset key={`${index}-${task.taskId}`}><legend>步骤 {index+1}</legend>
      ...
      <div className="resource-reference-list"><strong>步骤资源绑定</strong>{task.references.map((reference,referenceIndex)=><div className="resource-reference" key={referenceIndex}><label>类型<select value={reference.kind} onChange={event=>updateReference(index,referenceIndex,{kind:event.target.value as ExactReference["kind"]})}>{kinds.map(kind=><option key={kind}>{kind}</option>)}</select></label><label>资源 ID<input value={reference.resourceId} onChange={event=>updateReference(index,referenceIndex,{resourceId:event.target.value})}/></label><label>修订 ID<input value={reference.revisionId} onChange={event=>updateReference(index,referenceIndex,{revisionId:event.target.value})}/></label><button type="button" onClick={()=>updateTask(index,{references:task.references.filter((_,position)=>position!==referenceIndex)})}>移除此绑定</button></div>)}<button type="button" onClick={()=>updateTask(index,{references:[...task.references,{kind:"SKILL",resourceId:"",revisionId:""}]})}>添加精确资源绑定</button></div>

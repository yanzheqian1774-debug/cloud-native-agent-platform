...
export type Invocation = {admission?: {context_id:string;ready:boolean;reasonCode?:string;maximum_new_calls:number;cumulative_call_cap:number};adaptive?: {stop_reason: string; limits: {maximum_attempts: number; total_seconds:number}; attempts: {invocation_id:string;kind:string|null}[]};invocation: {bounded_new_calls?:number;target: {invocation_id: string}; request?: {answers: string[]}; submitted_at?: string}; result: {technical_status: string; reason?: string; kind: string | null; questions?: string[]; proposal?: Proposal | null; generated_at?: string}; facts_status: string};

    ...
    <div className="planning-layout"><main>
      ...
      <section className="planning-conversation-history" aria-label="已保存的规划对话">
        {history.conversation?.length ? history.conversation.map(message => <Fragment key={message.invocation_id}>
          {message.request.answers.length > 0 && <UserMessage authorLabel="已保存的规划输入" occurredAt={message.submitted_at}><span className="planning-context-note">该次请求携带的补充信息（可能包含前次上下文）</span><span style={{whiteSpace:"pre-wrap"}}>{message.request.answers.join("\n")}</span></UserMessage>}

  ...
  return <section className="planning-page">
    ...
    <div className="planning-layout"><main>
      ...
      <section className="planning-card"><h2>建议方案 <small>v{data.proposal.revision}</small></h2>{plan.stages.map((stage, index) => <div className="planning-stage" key={stage.stage_id}><div className="planning-stage-title"><span>{index + 1}</span><h3>{stage.title}</h3></div>{stage.task_ids.map(id => {
        ...
        return <details className="planning-task" key={id} open><summary>{task.title}<small>{id}</small></summary><p>职责：{task.responsibility}</p><p>数字员工：{employee?.name} <span className="planning-badge">{employee && labels[status(employee)]}</span></p><dl><dt>前置依赖</dt><dd>{task.depends_on.join("、") || "无"}</dd><dt>输入</dt><dd>{task.inputs.join("、")}</dd><dt>预期输出</dt><dd>{task.outputs.join("、")}（尚未生成）</dd></dl><table><thead><tr><th>资源</th><th>本任务用途</th><th>状态</th></tr></thead><tbody>{task.requirement_ids.map(rid => { const r = plan.requirements.find(r => r.requirement_id === rid)!; return <tr key={rid}><td><a href={`#requirement-${rid}`}>{r.kind} · {r.name}</a></td><td>{r.purpose}</td><td>{labels[status(r)]}</td></tr>; })}</tbody></table></details>;
      })}</div>)}</section>
      <section className="planning-card"><h2>资源需求与缺口</h2><p>资源独立维护，同一需求可供多个任务复用。匹配不代表已调用。</p>{plan.requirements.map(r => <details id={`requirement-${r.requirement_id}`} className="planning-resource" key={r.requirement_id}><summary>{r.name}<span>{labels[status(r)]} · {r.required ? "必要" : "可选"}</span></summary><p>{r.purpose}</p><p>{r.preparation}</p>{r.selected && <dl><dt>准确引用</dt><dd>{r.selected.resource_id} / {r.selected.revision_id}</dd><dt>摘要</dt><dd><code>{r.selected.digest}</code></dd></dl>}</details>)}</section>
    ...
    </main><aside><section className="planning-card"><h2>执行边界</h2><ul>{plan.boundaries.map(b => <li key={b}>{b}</li>)}</ul><p>本轮只确认规划，不执行采购操作。</p></section><section className="planning-card"><h2>资源状态</h2><div className="planning-counts"><div><b>{plan.requirements.length}</b>项需求</div><div><b>{matched}</b>已匹配</div><div><b>{missing.length}</b>必要缺口</div></div><button disabled={busy} onClick={() => void act("refresh")}>刷新资源状态</button><p>{data.snapshot ? `核对时间：${new Date(data.snapshot.checked_at).toLocaleString()}` : "尚未查询资源 owner，已选引用保持待核实。"}</p></section><section className="planning-card"><h2>方案版本</h2><label>查看历史建议<select value={data.proposal.revision} onChange={e => setParams({ revision: e.target.value })}>{history.proposals.map(p => <option key={p.revision} value={p.revision}>修订 {p.revision}</option>)}</select></label><p>历史版本与批准保留，修改语义需产生后继建议并重新确认。</p></section></aside></div>
  </section>;
}

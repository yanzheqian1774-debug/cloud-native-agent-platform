...
import {UserMessage, SystemMessage, ConversationFrame} from "../problems/ProblemConversation";
...
export function PlanningPage() {
  ...
  const changedTasks = previous ? [...new Set([...previous.semantics.tasks.map(t=>t.task_id), ...plan.tasks.map(t=>t.task_id)])].filter(id=>JSON.stringify(previous.semantics.tasks.find(t=>t.task_id===id))!==JSON.stringify(plan.tasks.find(t=>t.task_id===id))) : [];
  ...
  return <section className="planning-page journey-page">
    ...
    <div className="planning-layout"><main>
      ...
      <ConversationFrame startAtTop newMessageKey={`${proposalId}:${data.proposal.revision}`} composer={<PlanningDialogue key={`${proposalId}:${data.proposal.revision}`} problem={plan.target.problem.resource_id} source={data} disabled={data.proposal.revision !== latest}/>}><section className="planning-card planning-goal"><h2><span aria-hidden="true">◎</span> 目标与完成标准</h2><ul>{plan.business_rules.map(rule => <li key={rule}>{rule}</li>)}</ul><details className="journey-technical"><summary>问题与标准的准确版本</summary><dl><dt>问题</dt><dd>{plan.target.problem.revision_id}</dd><dt>成功标准</dt><dd>{plan.target.criteria.revision_id}</dd></dl></details></section>
      ...
      {history.conversation?.map(message => <section className="planning-card" key={message.invocation_id}>{message.request.answers.map((text,index)=><UserMessage key={index} occurredAt={message.submitted_at}>{text}</UserMessage>)}<p>消息提交时间：{formatTime(message.submitted_at)}</p>{message.questions?.map(q=><SystemMessage key={q}>{q}</SystemMessage>)}</section>)}
      </ConversationFrame>
    </main><aside>

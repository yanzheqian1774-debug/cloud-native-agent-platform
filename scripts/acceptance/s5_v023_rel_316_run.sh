...
artifact_dir="${1:-$(mktemp -d /tmp/s5-v023-rel-316.XXXXXX)}"
database_url="${REL_316_DATABASE_URL:?REL_316_DATABASE_URL_REQUIRED}"
public_port="${REL_316_PUBLIC_PORT:-18416}"
control_port="${REL_316_CONTROL_PORT:-18417}"
runtime_dir="$artifact_dir/runtime"
cert_path="$artifact_dir/rel-316.crt"
key_path="$artifact_dir/rel-316.key"
...
if [ "$ready" != true ]; then
  echo "REL_316_HARNESS_START_FAILED artifact_dir=$artifact_dir" >&2
  exit 1
fi

cd "$repo_root/console/frontend"
REL_316_WORKBENCH_URL="https://127.0.0.1:$public_port" \
REL_316_APPLICANT_CREDENTIAL="$applicant_credential" \
REL_316_ADMIN_CREDENTIAL="$admin_credential" \
PLAYWRIGHT_OUTPUT_DIR="$output_dir" \
npx playwright test \
  --config playwright.rel-316.config.ts \
  --workers=1 --retries=0 --forbid-only --reporter=json >"$raw_report"

echo "REL_316_ACCEPTANCE_PASSED artifact_dir=$artifact_dir"

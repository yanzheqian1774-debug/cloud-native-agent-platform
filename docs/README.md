# Documentation

Start with the root [README](../README.md) for project scope and the shortest
path to First Value.

## Users

- [Detailed local Kubernetes installation](../manifests/README.md)
- [Troubleshooting](troubleshooting.md)
- [Golden Engineering Demo](../examples/golden-engineering-demo/README.md)
- [Workflow Execution Console](../console/README.md)

## Releases

- [v0.1.0-alpha release notes](releases/v0.1.0-alpha.md)
- [Changelog](../CHANGELOG.md)

## Product and architecture

- [Current implementation boundary](engineering/CURRENT_IMPLEMENTATION.md)
- [Target architecture](../ARCHITECTURE.md)
- [Architecture decisions and known drift](../adr/README.md)
- [Product direction](../PRODUCT.md)
- [Roadmap](../ROADMAP.md)

Target architecture and roadmap documents include future direction. Source and
tests define current behavior; accepted ADRs define approved architecture.

## Contributors

- [Contributing guide](../CONTRIBUTING.md)
- [Repository map](engineering/REPOSITORY_MAP.md)
- [Definition of Done](engineering/DEFINITION_OF_DONE.md)
- [Architecture Gates](engineering/ARCHITECTURE_GATES.md)
- [Codex workflow](engineering/CODEX_WORKFLOW.md)

...
### D324-6 批准后实施 G1（2026-09-22）

将精确规划待准入对象与实际同案Problem/Criteria/目标摘要关联；新增追加式次数修订及独立决定，复用当前账号、配置、scope与Grant校验。预算owner在原policy锁内投影本次有效累计次数20，金额仍按原policy累计全部历史/UNKNOWN；旧323仍走原guard。无真实对象不得创建具体决定，无签发不得派发。迁移保持旧记录，更新writer兼容检查；回退须停writer并保留新增记录。定向验证精确对象/主体/配置、独立签发、过期撤销、19到20并发与第21次拒绝、金额不绕过、幂等及UNKNOWN阻断；正常门禁后再装配实际环境。

用户已确认新证书信任处理完成，独立新窗口可打开登录页；不再请求该步骤。代理仍需验证实际入口与安全返回，不刷新原业务标签。前候选4be343e的12项CI已全部成功；这不代表新增D324-6实现或真实闭环通过。

### D324-6 实施与故障验证

0038在0037独立准入owner下保存实际规划待准入对象与单一追加次数修订。待准入先保存精确目标/Criteria、请求承诺、资源快照和生成对象；不先claim实际调用后再把等待本人误投影为UNKNOWN。明确继续入口重验当前目标/模型及会话/精确Grant，原请求GET只读；一次调用后即使INVALID也不自动修正第二次。原policy不变、累计20、USD10及UNKNOWN占额仍在原锁下检查；来源context UNKNOWN也阻断后继。旧323仍走原guard，不继承新增修订。

配置预检发现当前55秒read timeout来自323精确案例修订，不能挪用给新问题。新324运行资料候选恢复原30秒配置，摘要58bdd7e3edcbe8fa4c41b6e08e8e82d949950f17373041349b35c21809208ec9与原批准逐字段一致；保留55秒历史文件/决定，不改变模型、价格、调用或金额policy。实际装配另记。

正常make check首轮被新测试格式阻断，格式修正；随后取消边界测试失败，端点已收到请求但客户端取消未抛异常。新增确定性测试先复现失败：disconnect探针吞掉外部取消投递，pending cancel仍存在。修正cancellable_request在探针后核验调用方取消意图，等待owner持久UNKNOWN再传播；定向新旧两项通过，未增超时或改弱断言。完整门禁随后2123 passed /372环境skipped /1 warning，167.37秒。原deadline失败证据仍保留；此修复不宣称消除全部机器时序风险。

实际隔离PG预算/准入6项通过，原规划调用和产品化回归42项通过；浏览器显式继续/刷新只读/125%等效视口通过，规划页用户消息按本任务要求右对齐。R30 P05、IAM07已实读，原图不具备精确规划准入卡片，沿原层次增补，不宣称整页视觉接受。本人证书步骤完成且新浏览器登录页读回成功；未使用实际审批账号或签发任何决定。
...
2026-09-22第3轮独立Grant已正式批准并复用，真实回复DRAFT_READY（估算USD0.029072）。供应商交付问题已通过正常入口创建DRAFT，并纠正具体演示日期的来源归属；尚不构成目标/标准确认。当前必要权限与真实对象、链接预检、资源范围提示集中登记于闭环矩阵及本机next-human-steps.md。后续在实际准入后继续标准、原调用来源后补、资源校验/本人审核、规划及Native。已完成模型调用不重跑；原history/UNKNOWN保护继续有效。

### 交付标准评价与绑定补齐（G1，2026-09-22）

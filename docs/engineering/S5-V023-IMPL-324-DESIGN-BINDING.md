...
## D324-7 task 审核页局部设计（编码前，2026-09-22）

实读R35 VERSION/CATALOG及R37 README；R37仅参考，不替换锁定图。`/authorization-admin?task=<实际request_id>` 对应R35/pages/IAM07.png，SHA256 `3df530f13c66b04aebbe56b1af7ce5513551e37052b6266480dc3a653d389dd7`，与R30/IAM07字节相同。图为合成设计，视觉接受未获授予。

本轮局部实现：沿用现有公共框架，左栏任务/合成数据/精确授权，右栏申请人与当前审批人、有效期及决定影响；只显示后端实际ACTIVE/PENDING状态，不搬用图中“核验通过”。决定区在内容流中可达，不用固定栏遮挡长内容。精确task URL自动只读加载，不要求手填ID；显式勾选范围再独立签发。图内计划确认/自动核验文案按D7区分为办理资格；不声称签发即确认Plan或执行。未实现拒绝记录按钮不仿造；提供查询、签发及已有签发撤销。普通视口/125%对照仍须实际验证。

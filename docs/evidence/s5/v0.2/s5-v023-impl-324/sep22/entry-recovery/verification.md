...
f37514f候选CI保留三项失败：Understanding Interaction三条断言暴露恢复摘要覆盖成功后的结构化理解内容（实现回归）；REL316与REL317均在原“业务问题与成功标准”审批标题定位失败，尚未进入Native。修正为仅未成功调用显示恢复摘要，保留原审批标题并单列具体事项。原断言不变，41项理解/规划交互和独立临时PG的REL316 HTTPS流程通过。REL316使用独立输出目录构建，未操作实际业务库；fixture临时库及自有服务已清理。

实际服务已在f37514f正常重启，身份代次与配置摘要保持、密码未轮换、无新迁移；登录/work/精确审批入口使用新证书显式校验的HTTPS GET均200，实际辅助构建资产GOzSASfV/CHoj_jNi。此为技术装配证据，浏览器安全提示尚未解除。进一步只读钥匙串定位：CN=127.0.0.1仍保存旧证书，09-22 09:32:44到期、SHA256 DE:EF:31:65:55:22:6D:F3:DA:30:6D:94:93:72:56:B2:F3:86:F6:9D:82:5D:30:65:25:DB:EB:09:AC:26:30:24；新证书尚非该旧信任项。没有改钥匙串或删除旧证据。兼容修正候选CI与最终运行读回另记。

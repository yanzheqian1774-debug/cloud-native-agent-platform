...
| S5-ARCH-018 | Bounded Product Continuity Persistence Architecture / ARCH G2 | `ACTIVE / AUTHORIZED / CHECKPOINT_A / AMENDED_PROPOSAL_READY_FOR_HUMAN_REVIEW` | Architecture/governance only; PostgreSQL is primary deployment persistence for new v0.2.2–v0.2.4 product-continuity domains; existing Evidence SQLite has a bounded explicit transition and SQLite/in-memory remain focused local/test adapters; no implementation, public API/CRD, State Plane, Tenant, HA, multi-region or production authority | Exact baseline `a6ec463a365b5f12e8fb64b0b84772a3beb0ae15`; branch `codex/s5-arch-018-bounded-product-continuity-persistence`; [decision](../../architecture/s5/v0.2/S5-ARCH-018-BOUNDED-PRODUCT-CONTINUITY-PERSISTENCE-V1.md); [Evidence](../evidence/s5/v0.2/s5-arch-018/README.md); prior SQLite-default proposal superseded by Human amendment | Allocation/G2 amendment: `HUMAN_CONFIRMED`; amended architecture acceptance, merge, Durable Integration and implementation allocation: `PENDING` | baseline/collision/content: `REPOSITORY_NATIVE / PR_NATIVE`; amended decision: `PROPOSED` | Human review of PostgreSQL amendment; no REL/implementation starts automatically |
